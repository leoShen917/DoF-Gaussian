...
def readColmapSceneInfo(path, images, eval, llffhold=8, scale=1.0):
    
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin") #sparse/0
    ...
    # positions1, colors1, normals1= fetchPly(ply_path)
    
    # pcd = BasicPointCloud(points=positions1, colors=colors1, normals=normals1)
    reading_dir = "images" if images == None else images
    cam_intrinsics[1].params[0] *= scale
    ...
    try:
        ...
        try:
            ...
            pcd = None
    # index = np.random.choice(np.arange(0,positions1.shape[0]),size=positions.shape[0]*10,replace=False)
    # positions = np.concatenate((positions,positions1[index]))
    # colors = np.concatenate((colors,colors1[index]))
    # colors = colors[positions[:,-1]>0]
    # normals = np.concatenate((normals,normals1[index]))
    # normals = normals[positions[:,-1]>0]
